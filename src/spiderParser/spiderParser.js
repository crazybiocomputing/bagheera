...
function readSpiderFile(file, type, callback, endianness) {
	try {
		...
		reader.onload = function(e) {
			...
			var imagePix = getContents(contents, type, endianness) ;
 ...
 */
function getContents(contents, type, endianness) {
	try {
		var pos = 0 ;
		var header = new DataView(contents, pos, defaultHeaderSize * sizeFloat) ;
		pos = defaultHeaderSize * sizeFloat;
		var fNsam = header.getFloat32(11 * sizeFloat, endianness) ;
		var fLabrec = header.getFloat32(12 * sizeFloat, endianness) ;
		...
		var ncol = header.getFloat32(11 * sizeFloat, endianness) ; //number of pixels per row
		var nrow = header.getFloat32(1 * sizeFloat, endianness) ;  //number of rows
		...
		if(type === 'image') {
			...
			for(var i = 0 ; i < nrow ; i++) {
				...
				for(var j = 0 ; j < ncol ; j++) {
					res[i][j] = image.getFloat32(pos * sizeFloat, endianness ) ;
					pos++ ;
				}
			}
			return  res ;
			...
			var nslice = header.getFloat32(0 * sizeFloat, endianness) ;
			...
			for(var k = 0 ; k < nslice ; k++) {
				...
				for(var i = 0 ; i < nrow ; i++) {
					...
					for(var j = 0 ; j < ncol ; j++) {
						res[k][i][j] = stack.getFloat32(pos * sizeFloat, endianness) ;
						pos++ ;
					}
				}
			}
